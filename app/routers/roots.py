...
from app.models.root_schemas import RootInput, RewriteRootInput, NthRootInput, RootResult
...
def simplify_root_route(data: RootInput):
    try:
        result, latex = roots.simplify_root(data.expression)
...
def rationalize_root_route(data: RootInput):
    try:
        result, latex = roots.rationalize_root(data.expression)
        return {"result": result, "latex": latex}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def expand_root_route(data: RootInput):
    try:
        result, latex = roots.expand_root(data.expression)
        return {"result": result, "latex": latex}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def combine_root_route(data: RootInput):
    try:
        result, latex = roots.combine_root(data.expression)
        return {"result": result, "latex": latex}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def rewrite_root_route(data: RewriteRootInput):
    try:
        result, latex = roots.rewrite_root(data.expression, data.target)
        return {"result": result, "latex": latex}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def nth_root_route(data: NthRootInput):
    try:
        result, latex = roots.nth_root(data.expression, data.degree)
        return {"result": result, "latex": latex}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
